...
test("ignores vendored packages and Desktop build outputs outside workspace patterns", async (t) => {
	const result = await check(t, {}, "", {
		"packages/desktop/package.json": JSON.stringify({ name: "desktop", private: true }),
		"packages/desktop/src/extensions/plugin/package.json": JSON.stringify({ name: "plugin" }),
		"packages/desktop/src/extensions/plugin/src/index.ts": 'import "plugin-only";',
		"packages/desktop/out/plugin/package.json": JSON.stringify({ name: "built-plugin" }),
		"packages/desktop/out/plugin/src/index.js": 'import "built-only";',
	});
	assert.equal(result.status, 0, result.stderr);
});
