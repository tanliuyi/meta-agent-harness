...
async function resolveSessionPath(
  ...
  const localSessions = await SessionManager.list(cwd, sessionDir)
  const localMatch =
    localSessions.find((s) => s.id === sessionArg) ??
    localSessions.find((s) => s.id.startsWith(sessionArg))
  ...
  const globalMatch =
    allSessions.find((s) => s.id === sessionArg) ??
    allSessions.find((s) => s.id.startsWith(sessionArg))
...
function validateSessionIdFlags(parsed: Args): void {
  ...
  if (conflictingFlags.length > 0) {
    console.error(
      chalk.red(`Error: --session-id cannot be combined with ${conflictingFlags.join(', ')}`)
    )
    process.exit(1)
  }
...
async function promptForMissingSessionCwd(issue: SessionCwdIssue): Promise<string | undefined> {
  console.error(chalk.yellow(formatMissingSessionCwdPrompt(issue)))
  return undefined
}
...
export async function main(args: string[], options?: MainOptions) {
  ...
  if (missingSessionCwdIssue) {
    await promptForMissingSessionCwd(missingSessionCwdIssue)
