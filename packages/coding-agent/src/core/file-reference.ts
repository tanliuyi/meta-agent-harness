...
export function extractFileReferenceQuery(textBeforeCursor: string): FileReferenceQuery | undefined {
...
export async function parseFileReferenceTokens(text: string, cwd: string): Promise<FileReferenceToken[]> {
...
export function removeFileReferenceTokens(text: string, tokens: Pick<FileReferenceToken, "from" | "to">[]): string {
	if (tokens.length === 0) {
		return text;
	}
...
function findUnquotedTokenStart(text: string): number {
	const lastDelimiter = Math.max(text.lastIndexOf(" "), text.lastIndexOf("\t"), text.lastIndexOf("\n"));
...
export function toDisplayFileArg(filePath: string, cwd: string): string {
	return formatPathRelativeToCwdOrAbsolute(isAbsolute(filePath) ? filePath : resolvePath(filePath, cwd), cwd);
}
