...
export class DefaultResourceLoader implements ResourceLoader {
  ...
  async reload(options?: ResourceLoaderReloadOptions): Promise<void> {
    ...
    const enabledExtensions = getEnabledPaths(
      resolvedPaths.extensions.filter(
        (resource) => !this.isExcludedExtensionSource(resource.metadata.source)
      )
    )
  ...
  private async loadCurrentExtensionSet(options: {
    ...
    const enabledExtensions = resolvedPaths.extensions
      .filter(
        (resource) => resource.enabled && !this.isExcludedExtensionSource(resource.metadata.source)
      )
