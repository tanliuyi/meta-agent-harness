...
Project skill`
      ...
      const baseTheme = JSON.parse(
        readFileSync(
          join(process.cwd(), 'src', 'modes', 'interactive', 'theme', 'dark.json'),
          'utf-8'
        )
...
export default function(pi) {
	...
	pi.registerCommand("user-only", {
      ...
      expect(
        extensionsResult.errors.some((e) => e.error.includes('Command "/deploy" conflicts'))
      ).toBe(false)
...
Project skill content`
      ...
      expect(
        loader
          .getAgentsFiles()
          .agentsFiles.some((file) => file.path === join(agentDir, 'AGENTS.md'))
      ).toBe(true)
      expect(
        loader.getAgentsFiles().agentsFiles.some((file) => file.path === join(cwd, 'AGENTS.md'))
      ).toBe(true)
      expect(loader.getExtensions().extensions).toHaveLength(0)
