...
describe('DefaultPackageManager', () => {
  ...
  describe('resolve', () => {
    it('should return no package-sourced paths when no sources configured', async () => {
      ...
      expect(
        result.skills.every(
          (r) => r.metadata.source === 'auto' && r.metadata.origin === 'top-level'
        )
      ).toBe(true)
    })
...
Content`
        ...
        writeFileSync(
          join(sharedThemesDir, 'shared.json'),
          JSON.stringify({ name: 'shared-theme' })
        )
    ...
    it('should resolve directory with package.json pi.extensions in extensions setting', async () => {
      ...
      expect(
        result.extensions.some((r) => r.path === join(pkgDir, 'extensions', 'clip.ts') && r.enabled)
      ).toBe(true)
      expect(
        result.extensions.some((r) => r.path === join(pkgDir, 'extensions', 'cost.ts') && r.enabled)
      ).toBe(true)
  ...
  describe('npmCommand', () => {
    ...
    it('should reconcile an existing git checkout to a pinned ref during install', async () => {
      ...
      vi.spyOn(managerWithInternals, 'runCommandCapture').mockImplementation(
        async (_command, args) => {
    ...
    it('should update git package dependencies with --omit=dev', async () => {
      ...
      vi.spyOn(packageManager as any, 'runCommandCapture').mockImplementation(
    ...
    it('should use npmCommand argv for npm root lookup and invalidate cached root when npmCommand changes', () => {
      ...
      expect(packageManager.getInstalledPath('npm:@scope/pkg', 'user')).toBe(
        join(root20, '@scope', 'pkg')
      )
      expect(runCommandSyncSpy).toHaveBeenNthCalledWith(1, 'mise', [
        'exec',
        'node@20',
        '--',
        'npm',
      ...
      expect(runCommandSyncSpy).toHaveBeenNthCalledWith(2, 'mise', [
        'exec',
        'node@22',
        '--',
        'npm',
    ...
    it('should install user npm packages into the pi-managed npm root', async () => {
      ...
      const runCommandSpy = vi
        ...
        .mockImplementation(async (...callArgs: unknown[]) => {
          ...
          writeFileSync(
            ...
            JSON.stringify({ name: 'pnpm-pkg', version: '1.0.0' })
          )
          writeFileSync(
            join(packagePath, 'extensions', 'index.ts'),
      ...
      expect(
        first.extensions.some(
          (r) => r.path === join(packagePath, 'extensions', 'index.ts') && r.enabled
        )
      ).toBe(true)
      expect(
      ...
      vi.spyOn(packageManager as any, 'runCommandSync').mockImplementation(
        (...callArgs: unknown[]) => {
          ...
          if (args.join(' ') === 'exec node@20 -- pnpm list -g --depth 0 --json') {
            return JSON.stringify([
              { path: pnpmRoot, dependencies: { 'pnpm-pkg': { path: packagePath } } }
            ])
          }
          throw new Error(`unexpected args ${args.join(' ')}`)
        }
  ...
  describe('source parsing', () => {
    ...
    it('should parse package source types from docs examples', () => {
      ...
      expect((packageManager as any).parseSource('https://github.com/user/repo@v1').type).toBe(
        'git'
      )
      expect((packageManager as any).parseSource('git:git@github.com:user/repo@v1').type).toBe(
        'git'
      )
      expect((packageManager as any).parseSource('ssh://git@github.com/user/repo@v1').type).toBe(
        'git'
      )
  ...
  describe('pattern filtering in pi manifest', () => {
    ...
    it('should expand positive glob manifest entries before collecting skills', async () => {
      ...
      expect(result.skills.some((r) => isEnabled(r, 'document-processor-api', 'includes'))).toBe(
        true
      )
    })
  })
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`
      ...
      expect(
        result.extensions.some((r) => pathEndsWith(r.path, 'subagent/index.ts') && r.enabled)
      ).toBe(true)
      expect(
        result.extensions.some((r) => pathEndsWith(r.path, 'standalone.ts') && r.enabled)
      ).toBe(true)
    ...
    it('should respect package.json pi.extensions manifest in subdirectories', async () => {
      ...
      expect(
        result.extensions.some((r) => pathEndsWith(r.path, 'custom/main.ts') && r.enabled)
      ).toBe(true)
    ...
    it('should handle mixed top-level files and subdirectories', async () => {
      ...
      expect(result.extensions.some((r) => pathEndsWith(r.path, 'simple.ts') && r.enabled)).toBe(
        true
      )
      expect(
        result.extensions.some((r) => pathEndsWith(r.path, 'complex/index.ts') && r.enabled)
      ).toBe(true)
    ...
    it('should skip subdirectories without index.ts or manifest', async () => {
      ...
      expect(result.extensions.some((r) => pathEndsWith(r.path, 'valid.ts') && r.enabled)).toBe(
        true
      )
  ...
  describe('offline mode and network timeouts', () => {
    ...
    it('should skip project npm update when installed version matches latest', async () => {
      ...
      writeFileSync(
        join(installedPath, 'package.json'),
        JSON.stringify({ name: 'example', version: '1.3.1' })
      )
      settingsManager.setProjectPackages(['npm:example@^1.0.0'])
    ...
    it('should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages', async () => {
      ...
      writeFileSync(
        join(userOldPath, 'package.json'),
        JSON.stringify({ name: 'user-old', version: '1.0.0' })
      )
      writeFileSync(
      ...
      writeFileSync(
        join(projectOldPath, 'package.json'),
        JSON.stringify({ name: 'project-old', version: '1.0.0' })
      )
      writeFileSync(
      ...
      const updateGitSpy = vi
        .spyOn(packageManager as any, 'updateGit')
    ...
    it('should skip refreshing temporary git sources when offline', async () => {
      ...
      expect(
        result.extensions.some((r) => pathEndsWith(r.path, 'extensions/index.ts') && r.enabled)
      ).toBe(true)
    ...
    it('should skip pinned packages when checking for updates', async () => {
      ...
      writeFileSync(
        join(installedNpmPath, 'package.json'),
        JSON.stringify({ name: 'example', version: '1.0.0' })
      )
      const parsedGitSource = (packageManager as any).parseSource('git:github.com/example/repo@v1')
      const installedGitPath = (packageManager as any).getGitInstallPath(
        parsedGitSource,
        'project'
      ) as string
    ...
    it('should wait for close before resolving captured stdout', async () => {
      ...
      let settled = false
      const capturePromise = managerWithInternals
