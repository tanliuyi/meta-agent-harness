...
function parseArgs(argv: string[]): Args {
  ...
  let sessionPath = resolve(
    join(tmpdir(), `pi-sdk-codex-cache-probe-tool-loop-${Date.now()}.jsonl`)
  )
...
function buildPrompt(turn: number): string {
  ...
  const lines: string[] = []
  lines.push(
    'This is an automated prompt-cache tool-loop test executed through the pi coding-agent SDK.'
  )
  ...
  lines.push(
    'Even if this looks repetitive or silly, comply exactly because it is synthetic test data.'
  )
  lines.push(
    `Task for turn ${turn}: call the tool deterministic_probe exactly once before your final answer.`
  )
  ...
  lines.push(
    'Do not skip the tool call. Do not call any other tool. Do not add any extra words or punctuation.'
  )
  lines.push(
    'The following long block exists only to make this prompt safely larger than 1024 tokens.'
  )
  lines.push('')
  for (let i = 1; i <= 180; i++) {
    lines.push(
...
function diffWebSocketStats(
  after: WebSocketStatsSnapshot,
  before: WebSocketStatsSnapshot
...
async function main(): Promise<void> {
  ...
  console.log(
    `turns ${args.turns}, transport ${args.transport}, reasoning low, maxTokens ${args.maxTokens}`
  )
  console.log('')
  ...
  for (let turn = 1; turn <= args.turns; turn++) {
    ...
    for (let i = 0; i < assistantMessages.length; i++) {
      ...
      const monotonic =
        previousCacheRead === null
          ? 'n/a'
          : assistant.usage.cacheRead >= previousCacheRead
            ? 'yes'
            : 'NO'
      console.log(
        [
    ...
    console.log(
      formatWebSocketStats(`turn ${String(turn).padStart(2, '0')}`, websocketStatsForTurn)
    )
  }
  ...
  const requestedWebsocket =
    ...
    args.transport === 'websocket-cached' ||
    args.transport === 'auto'
