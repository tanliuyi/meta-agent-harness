...
describe('AgentSession bash and persistence characterization', () => {
  ...
  it('records bash results immediately while idle', async () => {
    ...
    expect(harness.session.messages[harness.session.messages.length - 1]?.role).toBe(
      'bashExecution'
    )
  ...
  it('persists aborted assistant messages', async () => {
    ...
    const lastEntry =
      harness.sessionManager.getEntries()[harness.sessionManager.getEntries().length - 1]
