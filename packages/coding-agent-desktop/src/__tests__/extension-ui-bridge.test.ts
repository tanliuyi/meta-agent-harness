...
describe("ExtensionUiBridge", () => {
	...
	it("desktop webview panel 拒绝非法 URL", () => {
		...
		expect(() => desktop.updateWebviewPanel("bad", { source: { type: "url", url: "not a url" } })).toThrow(
			"Invalid desktop webview panel URL",
		);
		expect(() =>
	...
	it("desktop webview panel 支持 file source 和本地资源 URI 重写", () => {
		...
		try {
			writeFileSync(path.join(root, "icon.svg"), '<svg xmlns="http://www.w3.org/2000/svg"></svg>', "utf8");
			...
			const resourceEvents = projectionEvents.filter((item) => item.type === "extensionPanel.resourceRegistered");
	...
	it("desktop webview panel 支持 bundle source 的根路径资源", () => {
		...
		try {
			...
			writeFileSync(path.join(root, "index.html"), '<script type="module" src="/assets/index.js"></script>', "utf8");
			mkdirSync(assets, { recursive: true });
			writeFileSync(path.join(assets, "index.js"), "window.piPanel.post({ type: 'bundle-ready' })", "utf8");
			const events: WorkerEventEnvelope[] = [];
			const bridge = new ExtensionUiBridge("thread-1", (event) => events.push(event), root);
			const desktop = bridge.createDesktopContext();
	...
	it("desktop webview panel 拒绝 symlink 指向 localResourceRoots 外的资源", () => {
		...
		try {
			...
			writeFileSync(path.join(root, "index.html"), '<script src="./linked-secret.js"></script>', "utf8");
			const bridge = new ExtensionUiBridge("thread-1", () => {}, root);
			const desktop = bridge.createDesktopContext();
	...
	it("desktop webview panel 支持 asWebviewUri", () => {
		...
		try {
			...
			const event = events[0];
			if (event?.eventType !== "projection" || event.event.type !== "extensionPanel.resourceRegistered") {
	...
	it("desktop webview panel 的 asWebviewUri 拒绝越界资源", () => {
		...
		try {
			writeFileSync(path.join(root, "local.svg"), '<svg xmlns="http://www.w3.org/2000/svg"></svg>', "utf8");
			writeFileSync(path.join(outside, "secret.svg"), '<svg xmlns="http://www.w3.org/2000/svg"></svg>', "utf8");
			const bridge = new ExtensionUiBridge("thread-1", () => {}, root);
			const desktop = bridge.createDesktopContext();
