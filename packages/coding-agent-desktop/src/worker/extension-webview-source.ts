...
export type DesktopWebviewResourceRegistrationResult =
	| DesktopWebviewResourceRegistration
	| DesktopInlineWebviewResourceRegistration;

export type DesktopWebviewResourceReference = Omit<DesktopWebviewResourceRegistrationResult, "token">;
...
export function resolveDesktopWebviewPanelOptions(
	...
	return { ...options, source: resolveDesktopWebviewPanelSource(options.source, cwd, resourceUriFactory) };
}
...
export function resolveDesktopWebviewPanelPatch(
	...
	return { ...patch, source: resolveDesktopWebviewPanelSource(patch.source, cwd, resourceUriFactory) };
}
...
export function resolveDesktopWebviewPanelSource(
	...
	if (source.type === "file") {
		...
		const html = rewriteLocalResources(readFileSync(htmlFile, "utf8"), htmlFile, roots, path.dirname(htmlFile), resourceUriFactory);
		assertValidPortMappings(source.portMapping);
		return { type: "html", html, baseUrl: path.dirname(htmlFile), permissions: source.permissions, portMapping: source.portMapping };
	}
	...
	const html = rewriteLocalResources(readFileSync(htmlFile, "utf8"), htmlFile, roots, root, resourceUriFactory);
	assertValidPortMappings(source.portMapping);
	return { type: "html", html, baseUrl: root, permissions: source.permissions, portMapping: source.portMapping };
}
...
function createPathResourceUri(assetPath: string, roots: string[], resourceUriFactory: DesktopWebviewResourceUriFactory): string {
	const resolved = path.resolve(assetPath);
	assertReadableAsset(resolved, roots);
...
function createCssResourceUri(
	...
	const css = rewriteSourceMappingUrls(
		rewriteCssResourceUrls(readFileSync(resolved, "utf8"), resolved, roots, resourceRoot, resourceUriFactory),
		resolved,
		roots,
		resourceRoot,
		resourceUriFactory,
...
function createScriptResourceUri(
	...
	assertReadableAsset(resolved, roots);
	const script = rewriteSourceMappingUrls(readFileSync(resolved, "utf8"), resolved, roots, resourceRoot, resourceUriFactory);
...
function rewriteCssResourceUrls(
	...
	return css.replace(/url\(\s*(?:(["'])(.*?)\1|([^"')]+))\s*\)/gi, (match, quote, quotedValue, bareValue) => {
		const value = (quotedValue ?? bareValue ?? "").trim();
		if (!value || isExternalResource(value)) {
			return match;
		}
		const { suffix } = splitResourceReference(value);
		const resourceUri = createPathResourceUri(resolveAssetPath(value, cssBaseDir, resourceRoot), roots, resourceUriFactory);
		const nextQuote = quote ?? "";
		return `url(${nextQuote}${escapeAttribute(`${resourceUri}${suffix}`)}${nextQuote})`;
	});
}

function replaceQuotedAttribute(match: string, attribute: string, currentValue: string, nextValue: string): string {
	const attributePattern = new RegExp(`(\\b${attribute}\\s*=\\s*)(["'])${escapeRegExp(currentValue)}\\2`, "i");
	return match.replace(attributePattern, (_attributeMatch, prefix, quote) => `${prefix}${quote}${escapeAttribute(nextValue)}${quote}`);
}
...
function rewriteSourceMappingUrls(
	...
	return content.replace(/(sourceMappingURL=)([^\s*]+)/g, (match, prefix, value) => {
		...
		const resourceUri = createPathResourceUri(resolveAssetPath(sourceMap, baseDir, resourceRoot), roots, resourceUriFactory);
...
function rewriteScript(
	...
	const resourceUri = createScriptResourceUri(resolveAssetPath(src, baseDir, resourceRoot), roots, resourceRoot, resourceUriFactory);
...
function rewriteAttributeResource(
	...
	const { suffix } = splitResourceReference(value);
	const resourceUri = createPathResourceUri(resolveAssetPath(value, baseDir, resourceRoot), roots, resourceUriFactory);
...
function rewriteSrcset(
	...
	return value
		...
		.map((candidate) => {
			...
			const resourceUri = createPathResourceUri(resolveAssetPath(resource, baseDir, resourceRoot), roots, resourceUriFactory);
...
function rewriteLocalResources(
	...
	return html
		.replace(/<link\b(?=[^>]*\brel=["']stylesheet["'])(?=[^>]*\bhref=["']([^"']+)["'])[^>]*>/gi, (match, href) =>
		...
		.replace(/<(img|source|video|audio|track|image|use)\b[^>]*\b(src|href)=["']([^"']+)["'][^>]*>/gi, (match, _tag, attribute, value) =>
			rewriteAttributeResource(match, attribute, value, baseDir, roots, resourceRoot, resourceUriFactory),
		)
