...
export class SettingsManager {
	...
	setBranchSummaryReserveTokens(tokens: number): void {
		...
		this.globalSettings.branchSummary.reserveTokens = Math.max(0, Math.floor(tokens));
	...
	setRetryMaxRetries(maxRetries: number): void {
		...
		this.globalSettings.retry.maxRetries = Math.max(0, Math.floor(maxRetries));
	...
	setRetryBaseDelayMs(baseDelayMs: number): void {
		...
		this.globalSettings.retry.baseDelayMs = Math.max(0, Math.floor(baseDelayMs));
	...
	setHttpProxy(proxy: string | undefined): void {
		this.globalSettings.httpProxy = proxy;
	...
	setProviderRetryTimeoutMs(timeoutMs: number | undefined): void {
		...
		this.globalSettings.retry.provider = { ...(this.globalSettings.retry.provider ?? {}) };
		this.globalSettings.retry.provider.timeoutMs =
			timeoutMs === undefined ? undefined : Math.max(0, Math.floor(timeoutMs));
	...
	setProviderRetryMaxRetries(maxRetries: number | undefined): void {
		...
		this.globalSettings.retry.provider.maxRetries =
			maxRetries === undefined ? undefined : Math.max(0, Math.floor(maxRetries));
		this.markModified("retry", "provider");
		this.save();
	}
	...
	setProviderRetryMaxRetryDelayMs(maxRetryDelayMs: number): void {
		...
		this.globalSettings.retry.provider = { ...(this.globalSettings.retry.provider ?? {}) };
		this.globalSettings.retry.provider.maxRetryDelayMs = Math.max(0, Math.floor(maxRetryDelayMs));
		this.markModified("retry", "provider");
		this.save();
	}
