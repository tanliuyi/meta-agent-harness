...
Print resolved resource configuration for package resources.
Without -l, starts in global settings (~/${CONFIG_DIR_NAME}/agent/settings.json).
Use -l/--local to inspect project-local scope.
...
export async function handleConfigCommand(
	...
	console.log(`cwd: ${cwd}`);
	console.log(`scope: ${local ? "project" : "global"}`);
	console.log(`projectTrusted: ${settingsManager.isProjectTrusted() ? "yes" : "no"}`);
	console.log(`global extensions: ${globalResolvedPaths.extensions.length}`);
	console.log(`project extensions: ${projectResolvedPaths.extensions.length}`);
	console.log(`global skills: ${globalResolvedPaths.skills.length}`);
	console.log(`project skills: ${projectResolvedPaths.skills.length}`);
	return true;
}
