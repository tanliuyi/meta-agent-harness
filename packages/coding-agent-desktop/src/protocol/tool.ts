...
export function createDesktopFileChangeFromEditResult(input: DesktopEditFileChangeInput): DesktopFileChange | undefined {
...
function countDiffStats(diff: string | undefined, patch: string | undefined): { additions: number; deletions: number } {
	if (diff) {
		let additions = 0;
		let deletions = 0;
