...
import type { BuildHandlerOptions, BuildMiddleware, DocumentType } from '@smithy/types'
  ...
  ;(async () => {
    ...
    try {
      ...
      const inferenceMaxTokens =
        options.maxTokens ?? (isAnthropicClaudeModel(model) ? model.maxTokens : undefined)
      let commandInput = {
        modelId: model.id,
 ...
 */
function addCustomHeadersMiddleware(
  client: BedrockRuntimeClient,
  headers: Record<string, string>
): void {
  type AddBuildMiddleware = (
    middleware: BuildMiddleware<ServiceInputTypes, ServiceOutputTypes>,
    options: BuildHandlerOptions
  ) => void

  const middleware: BuildMiddleware<ServiceInputTypes, ServiceOutputTypes> =
    (next) => async (args) => {
  ...
  const addBuildMiddleware = client.middlewareStack.add as AddBuildMiddleware
  addBuildMiddleware(middleware, {
    step: 'build',
...
function supportsNativeXhighEffort(model: Model<'bedrock-converse-stream'>): boolean {
  ...
  return candidates.some(
    (s) => s.includes('opus-4-7') || s.includes('opus-4-8') || s.includes('fable-5')
  )
}
...
function getConfiguredBedrockCredentials(
  env?: ProviderEnv
): BedrockRuntimeClientConfig['credentials'] | undefined {
