...
import { openDesktopBrowser } from "./desktop-browser.ts"
import {
...
export async function authenticate(
  ...
  const operation = (async (): Promise<AuthStatus> => {
    ...
    try {
      ...
      try {
        // Desktop integration: open the authorization page in the session's
        // embedded browser when a Desktop browser host is present; fall back to
        // the system browser exactly like upstream when it is not.
        const desktopResult = await openDesktopBrowser(authorizationUrl)
        if (!desktopResult.ok) {
