...
import { mkdirSync, readFileSync, existsSync, rmSync, renameSync, chmodSync, writeFileSync } from 'fs';
...
const DESKTOP_AUTH_STORE_ENV = 'PI_MCP_ADAPTER_AUTH_STORE';
const DESKTOP_AUTH_FILE_ENV = 'PI_MCP_ADAPTER_AUTH_FILE';
const DESKTOP_AUTH_FILE_DEFAULT = 'mcp-oauth-credentials.json';
...
/**
 * Desktop fallback credential store: a single 0600 JSON file under the Pi
 * agent directory. The Desktop marketplace cannot ship the `@napi-rs/keyring`
 * native addon, so OAuth credentials fall back here (opt out by setting
 * PI_MCP_ADAPTER_AUTH_STORE=keyring).
 */
function getFileAuthStorePath(): string {
  const override = process.env[DESKTOP_AUTH_FILE_ENV]?.trim();
  if (override) return resolve(override);
  return getAgentPath(DESKTOP_AUTH_FILE_DEFAULT);
}

function readFileAuthEntries(): Record<string, string> {
  try {
    const raw = readFileSync(getFileAuthStorePath(), 'utf-8');
    ...
    throw new Error(`Failed to read Desktop OAuth credential file at ${getFileAuthStorePath()}`, { cause: error });
  }
}

function writeFileAuthEntries(entries: Record<string, string>): void {
  const filePath = getFileAuthStorePath();
  ...
  try {
    ...
    // chmod is not available on every platform/filesystem; the file still lands
    // under the owner's agent directory.
  }
...
const fileAuthSecretStore: AuthSecretStore = {
  read(account) {
    return readFileAuthEntries()[account];
  },
  write(account, payload) {
    const entries = readFileAuthEntries();
    entries[account] = payload;
    writeFileAuthEntries(entries);
  },
  remove(account) {
    const entries = readFileAuthEntries();
    if (!Object.prototype.hasOwnProperty.call(entries, account)) return;
    delete entries[account];
    if (Object.keys(entries).length === 0) {
      rmSync(getFileAuthStorePath(), { force: true });
    } else {
      writeFileAuthEntries(entries);
    }
  },
...
function isKeyringAvailable(): boolean {
  try {
    ...
    void new KeyringEntryClass(AUTH_SECRET_SERVICE, 'pi-mcp-adapter-availability-probe');
    return true;
  } catch {
    return false;
  }
...
function getAuthSecretStore(): AuthSecretStore {
  ...
  const override = process.env[DESKTOP_AUTH_STORE_ENV];
  if (override === 'file') return fileAuthSecretStore;
  if (override === 'keyring') return keyringAuthSecretStore;
  if (override !== undefined && override.trim() !== '') {
    throw new Error(`Unsupported ${DESKTOP_AUTH_STORE_ENV} value: ${override.trim()} (expected "file" or "keyring")`);
  }
  // Prefer the OS credential store when the native addon is loadable; fall
  // back to the Desktop file store when it is not (marketplace deployments of
  // this adapter never ship the native addon).
  if (isKeyringAvailable()) return keyringAuthSecretStore;
  console.warn('pi-mcp-adapter: OS credential store is unavailable; OAuth credentials will be stored in a 0600 file under the Pi agent directory. Set PI_MCP_ADAPTER_AUTH_STORE=keyring to force the OS store.');
  return fileAuthSecretStore;
}
