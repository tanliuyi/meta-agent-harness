...
/**
 * Model registry surface required for MCP sampling. `complete` is optional:
 * Pi hosts older than pi-coding-agent 0.84 (including the version bundled with
 * Meta Agent Desktop) do not expose model completion, so sampling must fail
 * with a clear message instead of crashing.
 */
export type SamplingModelRegistry = Pick<ModelRegistry, "getAvailable"> & {
  complete?: (
    model: Model<Api>,
    input: { systemPrompt?: string; messages: Message[] },
    options: {
      maxTokens?: number;
      temperature?: number;
...
export async function handleSamplingRequest(
  ...
  if (typeof options.modelRegistry.complete !== "function") {
    throw new Error("MCP sampling is not supported by this Pi host (model registry completion is unavailable)");
  }
