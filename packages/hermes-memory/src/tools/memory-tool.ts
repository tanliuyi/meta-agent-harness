...
export function registerMemoryTool(
  ...
  pi.registerTool({
    ...
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      ...
      const currentProjectStore = project?.store ?? projectStore;
      const currentProjectId = project?.info.id ?? projectName;
      ...
      const activeStore = rawTarget === "project" ? currentProjectStore : store;

      if (rawTarget === "project" && !currentProjectStore) {
        return {
      ...
      switch (action) {
        case "add":
          ...
          if (rawTarget === "failure") {
            ...
            if (result.success) {
              syncWarning = await syncAddToSqlite(rawTarget, content, memoryCategory, failure_reason, dbManager, currentProjectId);
            }
          } else {
            result = await store_.add(target, content);
            if (result.success) {
              await syncEvictionsFromSqlite(rawTarget, result.evicted_entries, dbManager, currentProjectId);
              syncWarning = await syncAddToSqlite(rawTarget, content, undefined, undefined, dbManager, currentProjectId);
            }
          }
          break;
        ...
        case "replace":
          ...
          if (result.success) {
            if (!result.updated_entry) {
              syncWarning = "Saved to Markdown, but the normalized replacement was unavailable for SQLite sync.";
            } else {
              syncWarning = await syncReplaceToSqlite(rawTarget, old_text, result.updated_entry, dbManager, currentProjectId);
            }
          }
          break;
        ...
        case "remove":
          ...
          if (result.success) {
            syncWarning = await syncRemoveFromSqlite(rawTarget, old_text, dbManager, currentProjectId);
          }
          break;
