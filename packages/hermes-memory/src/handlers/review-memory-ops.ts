...
async function syncReplace(
  ...
  if (!dbManager) return
  const failure = rawTarget === 'failure' ? parseFailureMemoryContent(newContent) : null
...
export async function applyReviewOperations(
  ...
  for (const op of operations) {
    ...
    const memoryTarget =
      rawTarget === 'project' ? 'memory' : rawTarget === 'failure' ? 'failure' : rawTarget
    ...
    switch (op.action) {
      ...
      case 'replace': {
        ...
        if (result.success && result.updated_entry) {
          await syncReplace(rawTarget, op.old_text, result.updated_entry, dbManager, projectName)
          appliedCount++
        } else {
          skippedCount++
        }
