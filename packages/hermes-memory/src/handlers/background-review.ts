...
export function setupBackgroundReview(
  ...
  pi.on("turn_end", async (event, ctx) => {
    ...
    const promptInput: ReviewPromptInput = {
      ...
      currentProject: currentProjectStore ? currentProjectStore.getMemoryEntries().join("\n§\n") : null,
    };
    ...
    const runReview = async (): Promise<void> => {
      ...
      const subprocessResult = await runSubprocessReview(pi, subprocessPrompt, config, execChild, active?.cwd);
