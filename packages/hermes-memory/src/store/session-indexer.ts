...
export function parseSessionManagerSnapshot(
...
function getStoredSessionFileMetadata(
  ...
  return dbManager
    .getDb()
    .prepare('SELECT size, mtime_ms FROM session_files WHERE path = ?')
    .get(filePath) as { size: number; mtime_ms: number } | undefined
}

function storedSessionFileMatches(
  dbManager: DatabaseManager,
...
export function indexChangedSessions(
  ...
  for (const metadata of changed) {
    ...
    try {
      ...
      result.errors.push(
        `Error indexing ${metadata.path}: ${err instanceof Error ? err.message : String(err)}`
      )
    }
  }
...
function getLastBackfillTimestamp(dbManager: DatabaseManager): string | null {
  ...
  const row = db
    ...
    .get(LAST_SESSION_BACKFILL_KEY) as { value: string } | undefined
