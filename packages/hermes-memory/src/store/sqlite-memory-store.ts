...
export function parseFailureMemoryContent(text: string): ParsedFailureMemoryContent {
  ...
  const category = categoryMatch && FAILURE_CATEGORY_SET.has(categoryMatch[1] as MemoryCategory)
  ...
  for (const segment of segments) {
    if (segment.startsWith('Failed: ') && !failureReason) failureReason = normalizeNullable(segment.slice(8));
    else if (segment.startsWith('Tool state: ') && !toolState) toolState = normalizeNullable(segment.slice(12));
    else if (segment.startsWith('Corrected to: ') && !correctedTo) correctedTo = normalizeNullable(segment.slice(14));
    else if (segment.startsWith('Project: ') && !project) project = normalizeNullable(segment.slice(9));
    else contentSegments.push(segment);
  }

  return { content: contentSegments.filter(Boolean).join(' — '), category, failureReason, toolState, correctedTo, project };
}
