...
function normalizedSkillSource(content: string): string {
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (await exists(targetPath)) {
          ...
          if (!targetStat.isFile()) throw new Error(`Migration target is not a regular file: ${targetPath}`);
          const targetRaw = await fs.readFile(targetPath, "utf-8");
          if (normalizedSkillSource(raw) === normalizedSkillSource(targetRaw)) {
            await fs.rm(legacyPath, { force: true });
          } else {
            result.warnings.push(`${file}: conflicts with ${targetPath}; preserved both files`);
          }
          result.skipped++;
          continue;
        }
