...
import { formatFailureMemoryContent, parseFailureMemoryContent } from "./sqlite-memory-store.js";
...
export class MemoryStore {
  ...
  async addFailure(content: string, options: {
    ...
    return this.withWriteLock(() =>
      this._add("failure", failureText, undefined, 1, "Failure memory saved: " + options.category)
    );
  }
  ...
  private async replaceUnlocked(target: "memory" | "user" | "failure", oldText: string, newContent: string): Promise<MemoryResult> {
    ...
    const today = new Date().toISOString().split("T")[0];
    const updatedEntry = target === "failure"
      ? formatFailureMemoryContent(newContent, parseFailureMemoryContent(decoded.text))
      : newContent;
    const encoded = this.encodeEntry(updatedEntry, decoded.created, today);
    ...
    return { ...this.successResponse(target, "Entry replaced."), updated_entry: updatedEntry };
  }
  ...
  private async withWriteLock<T>(operation: () => Promise<T>): Promise<T> {
    await fs.mkdir(this.memoryDir, { recursive: true });
    const release = await lockfile.lock(this.memoryDir, {
      realpath: false,
      stale: 30_000,
      retries: { retries: 20, minTimeout: 10, maxTimeout: 100 },
    });
    try {
      // Every writer refreshes after acquiring the cross-process lock so it
      // merges with the latest authoritative Markdown state.
      await this.loadFromDisk();
  ...
  private buildFailureMemoryText(content: string, options: {
    ...
    return formatFailureMemoryContent(content, options);
  }
