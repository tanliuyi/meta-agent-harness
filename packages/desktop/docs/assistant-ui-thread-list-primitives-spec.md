...
- `DesktopThreadActions.open/create/rename/archive/remove`；
...
ThreadListPrimitive.New click
  ...
  -> assistant-ui 清空并 hydrate 新 thread
  -> dispatch thread-created
  -> DesktopState / adapter props 同步
```

Project 不可用或未选择时 New 必须 disabled。不得创建 assistant-ui-only 临时 thread。
...
- switch/create 期间禁用重复的同类 action；
...
- switch/create 恢复 committed thread；
- rename 保留原 title；
...
- New 还需要 active/available Project；
...
4. 不出现双 attach、双 create、双 archive 或双 delete。
5. `DesktopState.threadId`、adapter `threadId` 和 assistant-ui `mainThreadId` 最终一致。
6. switch/create 失败恢复 committed thread，Workbench 不进入错误 session。
