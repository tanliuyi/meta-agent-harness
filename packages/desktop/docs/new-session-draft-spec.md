...
- `/` command suggestions 在 materialize 前不可用；
...
| draft models/defaults/readiness | main 只读 DraftSessionConfig + Desktop reducer cache | 否 |
...
3. 返回当前可用模型、默认模型、该模型支持的 thinking levels 和 readiness；
...
draft Project 非空时使用 `{ projectId, commands: [] }`：

- `@` 文件建议可用；
- `/` session/extension commands 不显示；
- materialize 后由真实 snapshot 恢复完整 commands。
...
`getDraftConfig()` 是只读配置查询，不得改变 ProjectStore active Project 或 session catalog。模型默认值与 thinking clamp 必须复用 Pi 创建路径的解析规则，不能在 renderer 复制一套 fallback。
...
6. draft 显示模型与 thinking 选择，且选择过程不创建 session。
...
draft 必须显示 model/thinking，但只持有最小 `DraftSessionConfig`，不伪造 `SessionControlState`。配置查询与首次 create 必须复用同一 Pi 模型解析语义；未来若模型来源加入 extension 动态 provider，需要扩展只读配置查询，仍不得通过隐藏 session 获取列表。
