...
- 初始定位、run-start 定位和 ResizeObserver 跟随；
- normal-flow spacer；
- 自定义 ScrollToBottom；
...
- `stickyRef`：高频滚动与 ResizeObserver 使用；
...
| pinned 时内容 ResizeObserver | pinned | `scrollTop = scrollHeight` | pinned |
...
pinned 到底部时，流式内容增长会触发两种滚动写入：

1. virtualizer 因真实高度与估算高度不同而修正 offset；
2. ResizeObserver 把 viewport 重新贴到底部。

两者并发会造成上下 rubber-band。自定义 `scrollToFn` 必须在以下条件同时成立时忽略向上的测量修正：

- `stickyRef.current === true`；
- 当前距离底部小于阈值；
- virtualizer 请求的 offset 小于当前最大 scroll offset。

其余情况使用无动画 `element.scrollTo(0, offset)`。
...
3. pinned 时由 content ResizeObserver 负责最终底部贴合。
4. detached 时允许 TanStack 默认保持 viewport anchor。
5. 不在流式 delta effect 中直接调用 `measure()`；DOM ResizeObserver 是唯一高度变更入口。
...
- `getItemKey(turn.id)` 保留未变化 turn 的测量缓存；
...
5. 实现自定义 `scrollToFn`、ResizeObserver 和 run-start jump。
...
- detached 状态不被内容 ResizeObserver 拉回底部。
...
| streaming 持续 remeasure | 高频 scroll writes | pinned ref、custom `scrollToFn`、ResizeObserver 最终校正 |
