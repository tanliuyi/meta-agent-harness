...
| extension editor | `ExtensionUIContext.setEditorText/getEditorText/pasteToEditor` 与当前 Composer | Desktop 双向同步；扩展命令按 revision 单次应用 |
...
### 18.1 Extension editor

Pi `ExtensionUIContext` 的 `setEditorText()`、`pasteToEditor()` 和 `getEditorText()` 指向同一个编辑器语义。Desktop 必须把 assistant-ui Composer 与 main `HostUi` 双向同步：

- 用户输入、发送后的清空、queue clear 恢复和 error reseed 产生的 Composer 文本变化必须同步到当前 SessionRuntime，供扩展同步调用 `getEditorText()`；
- extension 发出的 `setEditorText()` / `pasteToEditor()` 必须携带单调递增的 `editorRevision`，renderer 仅在 target 或 revision 改变时应用；
- 普通 control revision、stream delta、queue update 或 React runtime identity 变化不得重复应用旧 `editorText`，否则会覆盖用户草稿；
- renderer 回传文本只更新 HostUi 当前 editor value，不递增 extension command revision，也不反向发布 control；
- thread generation 和显式 project/thread ID 必须隔离迟到同步，禁止把旧线程草稿写入新线程。

该同步只维护 Pi 已公开的 extension editor 内存状态，不写入 timeline 或 `session.jsonl`。
...
- extension editor 命令按 revision 单次应用，Composer 变化可由 `getEditorText()` 同步读取；
- queue clear 恢复文本不会被旧 extension editor control 覆盖；
...
16. draft、attachments、cancel、HostUi、extension editor 双向同步、model/thinking、Workbench 和 thread CRUD 通过回归。
...
- [x] ExtensionUI editor 与 Composer 双向同步，旧 control 不覆盖新草稿；
