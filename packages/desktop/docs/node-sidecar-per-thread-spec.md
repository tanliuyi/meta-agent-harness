...
Desktop 不在 session 启动时自动安装或 rebuild。当前 runtime npm tree 缺失时，底层返回包含 runtime identity 的结构化错误；renderer 向客户显示“需要更新扩展”，并在执行前说明会联网且可能运行扩展安装脚本。用户明确确认后，main 使用 manifest 绑定的 Node、npm CLI、Pi entry 和 runtime fingerprint，只安装错误标识的 npm package；成功后自动重启 Desktop。技术路径和 ABI 信息只写日志。git package 与本地 extension 首期不提供 native dependency 隔离保证。
