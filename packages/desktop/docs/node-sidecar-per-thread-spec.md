...
- extension source、启用状态和发现顺序；
- extension/node_modules；
- model、auth、settings、prompt、skill 和 session JSONL 语义。

Desktop 不复制、投影、安装或重建 extension 依赖，不修改共享 npm tree，也不加载 Electron native addon。
...
Desktop 不执行 lifecycle script approval，也不在缺依赖时自动安装或 rebuild；缺依赖直接返回可诊断错误。扩展安装和依赖变更由 Pi CLI/用户管理，Desktop 下次启动或新建 worker 时读取最新共享目录。
...
- Desktop 与 Pi CLI 看到同一 extension/node_modules；
