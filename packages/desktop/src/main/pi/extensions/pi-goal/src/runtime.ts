...
export class GoalRuntime {
  ...
  private readonly goalStateListeners = new Set<(snapshot: GoalStateSnapshot) => void>();
  legacyQueueState?: LegacyQueueState;
  ...
  addGoalStateListener(listener: (snapshot: GoalStateSnapshot) => void): () => void {
    this.goalStateListeners.add(listener);
    return () => this.goalStateListeners.delete(listener);
  }
  ...
  private publishGoalState(snapshot: GoalStateSnapshot) {
    ...
    for (const listener of this.goalStateListeners) {
      try {
        listener(snapshot);
      } catch {
        // UI publication must not interrupt canonical Goal persistence.
      }
    }
  }
