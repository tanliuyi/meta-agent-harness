...
### US-5：在 sidebar 中快速识别当前 leaf

作为用户，我希望 sidebar 能告诉我当前对话内部处于哪个 branch/leaf，避免误以为切到了另一个对话。

验收：

- thread item 可以显示轻量 leaf 摘要，例如 `Current: assistant: ...`。
- 可展开 thread item 查看少量 leaf 快捷入口。
...
- 最近 leaf：按最近 navigate / append 活动维护 UI cache。
...
- thread item 可展开显示 current leaf / recent branches / labeled leaves。
- leaf 点击调用 `navigateTree`。
- 完整 tree 仍保留在 Session Panel。

验收：

- Sidebar 不展示无限完整 tree。
