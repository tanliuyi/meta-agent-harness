...
import { desktopWebviewHostStylesheetPath, type ExtensionAPI } from "@earendil-works/pi-coding-agent";
...
export default function desktopWebviewPanelExample(pi: ExtensionAPI) {
	...
	pi.on("session_start", (_event, ctx) => {
		...
		ctx.desktop.registerWebviewPanel("desktop-webview-demo", {
			...
			title: "Webview Demo",
			order: 50,
			source: { type: "html", html: createPanelHtml(iconUri, hostStylesheetUri), permissions: { enableScripts: true } },
		});
	...
	pi.on("desktop_panel_restore", (event, ctx) => {
		...
		ctx.desktop.postPanelMessage(event.panelId, { type: "state", message: "Panel restored", state: event.state });
	});
