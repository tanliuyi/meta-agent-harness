...
describe('preload API surface', () => {
  it('只暴露 window.api，不暴露 electronAPI 或 ipcRenderer 原对象', () => {
    ...
    expect(source).toContain('openSessionMessageFeed: (input) =>')
    expect(source).toContain('codingAgentChannels.openSessionMessageFeed')
    expect(source).toContain('closeSessionMessageFeed: () =>')
    expect(source).toContain('codingAgentChannels.closeSessionMessageFeed')
    expect(source).toContain('onSessionAgentEvent: (listener) =>')
    expect(source).toContain('codingAgentChannels.sessionAgentEvent')
