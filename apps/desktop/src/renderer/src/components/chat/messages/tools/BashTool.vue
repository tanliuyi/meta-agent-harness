...
const summary = computed(() =>
  joinSummary([
    getStringArg(args.value, 'command'),
    getNumberArg(args.value, 'timeout')
      ? `timeout=${getNumberArg(args.value, 'timeout')}s`
      : undefined
  ])
)
...
<template>
  <BaseTool name="Bash" :summary="summary" :result="result" :status="status" :is-error="isError" />
</template>
